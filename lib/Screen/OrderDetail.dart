                  ...
                  children: [
                    Text("${getTranslated(context, 'TAX_AMOUNT')!} :",
                        style: Theme.of(context).textTheme.button!.copyWith(
                            color: Theme.of(context).colorScheme.lightBlack2)),
                    Text(
                        ' ${getPriceFormat(context, double.parse(widget.model!.taxAmount!))!}',
                        style: Theme.of(context).textTheme.button!.copyWith(
                            color: Theme.of(context).colorScheme.lightBlack2))
                  ],
                ),
...
/*  DwnInvoice() {
  ...
  void openBottomSheet(BuildContext context, OrderItem orderItem) {
    curRating = double.parse(orderItem.userReviewRating!);
            ...
            child: MaterialButton(
              ...
              onPressed: () {
                Future.delayed(Duration.zero)
                    .then((value) => commentTextFieldKey.currentState!.save());

                if (curRating != 0.0) {
                  Navigator.pop(context);
                  setRating(curRating, updatedComment, reviewPhotos,
                      orderItem.productId);
                } else {
                  Navigator.pop(context);
